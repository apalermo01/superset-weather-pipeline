...
class DataExtraction(TypedDict):
    data: list
    schema: str
    table: str
    id_cols: list
...
@db.insert_into_raw
def populate_stations_by_state(
    states: list, use_cache: bool = True, limit: Optional[int] = 500
) -> Optional[ApiCallResults]:
    try:
        results = apis.get_stations_by_state(states, use_cache=use_cache, limit=limit)
        data = results["data"]
    ...
    return {"data": structured_data, "url": results["url"], "cached": results["cached"]}
...
def get_latest_observations_by_station(station_id: str) -> Optional[ApiCallResults]:
    try:
        print("getting observations for ", station_id)
        results = apis.get_latest_observations(station_id)
        data = results["data"]
    except Exception as e:
        logger.error(e)
        return None
...
#     }
#
#     update_cols = [k for k in record if k != 'observation_id']
#
#     return {
#         "data": record,
#         "schema":
